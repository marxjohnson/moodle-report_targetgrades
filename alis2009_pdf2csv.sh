...
sed 's/[ ]\{2,\}/|/' Equations.txt | sed 's/[ ]\{2,\}/|/' | sed 's/[ ]\{2,\}/|/' | sed 's/[ ]\{2,\}/|/' | sed 's/[ ]\{2,\}/|/' | sed 's/Subject|.*$//' | sed 's/^.*|Page.*$//' | sed 's/|Alis Subject Level Regression Equations [0-9]\{4\}//' | sed '/^[]*$/d' | sed '1,2d' | sed '/^Technolog$/d' | sed 's/\(Information And Communications\)/\1 Technology/' > Equations.txt